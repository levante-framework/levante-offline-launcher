<template>
  <div class="page">
    ...
    <p class="muted">
      While online, sign in as the site's proctor, pick an administration, and download everything the
      device needs to assess its children offline: roster, task settings, and stimuli.
    </p>
    <div class="row">
    ...
    <div class="card" v-if="session">
      ...
      <div class="row" style="margin-top: 12px">
        <button type="button" class="primary big" :disabled="!selectedId || busy || !online || !hasVault" @click="provision">
    ...
    <div class="card">
      ...
      <table v-if="packs.length">
        <thead>
          <tr><th>Administration</th><th>Locale</th><th>Children</th><th>Tasks</th><th>Files</th><th>Status</th><th></th></tr>
        </thead>
        <tbody>
...
import type { PackRecord, PackTaskConfig, RosterEntry } from '../offline/types';
...
async function provision() {
  ...
  try {
    const res = await callFunction<ProvisionResult>('provisionOfflinePack', { administrationId: selectedId.value });
    const p = res.pack;
    packId = p.packId;
    ...
    message.value = `Provisioned "${done.name}": ${done.children.length} children, ${done.tasks.length} tasks, ${done.fileCount} files (${(done.totalBytes / 1e6).toFixed(1)} MB). This device can now assess offline.`;
  } catch (err) {
    error.value = err instanceof Error ? err.message : String(err);
