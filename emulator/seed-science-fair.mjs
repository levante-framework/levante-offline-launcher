...
async function main() {
  ...
  await db.collection('administrations').doc(ADMINISTRATION_ID).set({
    ...
    classes: [],
    groups: [],
    readOrgs: { districts: [SITE_ID], schools: [SCHOOL_ID], classes: [], groups: [] },
    minimalOrgs: { districts: [SITE_ID], schools: [SCHOOL_ID], classes: [], groups: [] },
    assessments,
    legal: {},
    testData: true,
  });
  ...
  for (const child of children) {
    ...
    await db.collection('users').doc(child.uid).collection('assignments').doc(ADMINISTRATION_ID).set({
      ...
      progress,
      assigningOrgs: { districts: [SITE_ID], schools: [SCHOOL_ID], classes: [], groups: [] },
      readOrgs: { districts: [SITE_ID], schools: [SCHOOL_ID], classes: [], groups: [] },
